...
删除”请求本身不是确认。创建和修改仍必须先生成规定的静态 PNG 预览，再等待下方确认判定规则允许的确认回复（包括
“确认创建”或
“确认修改”。删除/取消必须展示准确的日历、事件标题/ID、本地时间、时区和删除范围（单次事件、
单个循环实例或整个循环系列），再等待下方确认判定规则允许的确认回复（包括“确认删除”或“确认取消”）。字段、候选事件、时间、日历或
...
仅在已经展示完整提案（以及创建/修改所需预览）并等待确认时，按下面规则判断用户回复；这套判定只改变确认词识别，
不改变提案、冲突检查、预览或写入流程：
